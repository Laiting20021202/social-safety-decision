...
from realtime_safety.types import Detection2D, FramePacket, PipelineSnapshot, PointCloudFrame, RecommendedAction, SafetyLevel
...
class RealtimePipeline:
    def __init__(
        ...
        self.config = config
        self.segmentation = segmentation_backend or create_segmentation_backend(config.segmentation, config.device)
        ...
        while not self._stop_event.is_set():
            ...
            try:
                if use_st4r:
                    ...
                    last_st4r = time.perf_counter()
                    if frame.frame_index - anchor.frame_index > 30:
                        anchor = frame
                        self.st4r.set_anchor(frame)
                elif use_depth:
                    with self._gpu_lock:
                        cloud = self.depth.infer(frame)
                    last_depth = time.perf_counter()
                else:
                    continue
    ...
    def _renderer_worker(self) -> None:
        ...
        while not self._stop_event.is_set():
            ...
            signature = (
                display_frame.frame_index if display_frame is not None else -1,
                cloud_version,
                id(safety),
            )
            if display_frame is None or signature == last_signature:
                self._stop_event.wait(0.01)
                continue
            ...
            profile = self.adaptive.observe(performance)
            display_detections = perception.detections if perception is not None else []
            annotated = draw_video_overlay(display_frame.bgr, display_detections, level, performance)
            is_new_display_frame = display_frame.frame_index != last_displayed_frame_index
            ...
            if (
                ...
                self._source_done.set()
            if not self._calibrated and self._segmentation_ms is not None and self._reconstruction_ms is not None:
